...
import com.pnowak.personalbudget.backend.repository.BudgetRepository
...
@Service
class BudgetServiceImpl(private val jdbcTemplate: NamedParameterJdbcTemplate,
                        private val budgetRepository: BudgetRepository,
                        private val categoryRepository: CategoryRepository) : BudgetService {
    override fun getMonthYearBudget(month: Int, year: Int, userId: Long?): MutableList<Budget?> {
        val sql = "SELECT\n" +
                ...
                "GROUP BY c.id, c.title, c.id, c.id_parent, bp.amount, bp.id\n"
    ...
    override fun createBudget(createBudgetDTO: CreateBudgetDTO): Long {
        val category = categoryRepository.findById(createBudgetDTO.categoryId)
        return budgetRepository.save(BudgetPlan(id = createBudgetDTO.budgetPlanId, amount = createBudgetDTO.plan, monthYear = createBudgetDTO.monthYear, category = category.get())).id!!
    }
}
